...
export default function ChallanCalculator() {
  ...
  useEffect(() => {
    ...
    const fetchMetadata = async () => {
      try {
        const res = await fetch('http://127.0.0.1:8000/api/v1/calculator/metadata', {
          headers: {
            'Authorization': 'Bearer YOUR_MASTER_API_KEY_HERE' // Placeholder, but valid if backend bypasses or uses local auth
          }
        });
  ...
  const handleCalculate = async () => {
    ...
    try {
      const res = await fetch('http://127.0.0.1:8000/api/v1/calculator/calculate', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer YOUR_MASTER_API_KEY_HERE'
        },
        body: JSON.stringify({
    ...
    <div className="flex flex-col h-full bg-[#1a1a1a] rounded-xl p-6 overflow-y-auto">
      ...
      <div className="space-y-4 mb-6">
        ...
        <div>
          ...
          >
            {jurisdictions.map(j => (
              <option key={j.id} value={j.id} className="bg-gray-800">{j.name} ({j.type})</option>
            ))}
          </select>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            >
              ...
              <option value="LMV" className="bg-gray-800">Light Motor Vehicle (LMV)</option>
              <option value="HMV" className="bg-gray-800">Heavy Motor Vehicle (HMV)</option>
            </select>
          </div>
          ...
          <div className="flex items-center mt-6">
            <label className="flex items-center cursor-pointer">
              <input 
                ...
                className="w-5 h-5 rounded border-white/10 bg-white/5 text-[#7FFFD4] focus:ring-[#7FFFD4] focus:ring-offset-gray-900"
              />
        ...
        <button 
          ...
          disabled={isLoading || !selectedViolation || !selectedJurisdiction}
              ...
              {results.map((r, idx) => (
                <div key={idx} className="bg-white/5 border border-white/10 rounded-lg p-4">
                  ...
                  <div className="mt-4 pt-3 border-t border-white/10 text-xs text-gray-500">
                    <div>Legal Section: {r.legal_section_id}</div>
