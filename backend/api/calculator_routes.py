...
async def get_calculator_metadata(db: AsyncSession = Depends(get_db)):
    """
    Returns available violations and jurisdictions to populate frontend dropdowns.
    """
    try:
        ...
        return CalculatorMetadataResponse(
            violations=[
                ViolationMetadata(id=v.id, violation_code=v.violation_code, name=v.name) 
                for v in violations
            ],
            jurisdictions=[
                JurisdictionMetadata(id=j.id, name=j.name, type=j.type, parent_id=j.parent_id) 
...
async def calculate_fine(
    ...
    """
    Calculates the applicable fine based on deterministic rule logic.
    """
    try:
